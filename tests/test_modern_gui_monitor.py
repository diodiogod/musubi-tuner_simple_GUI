...
def test_stop_after_current_epoch_waits_for_the_following_epoch_header():
    class Process:
        def __init__(self):
            self.signals = []
    ...
    armed = supervisor.stop_after_current_epoch(True)
    assert armed["stop_after_epoch"] == 1

    supervisor._append_log("output", "epoch 1/4")

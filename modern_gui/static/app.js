...
function renderComparison(index) {
  ...
  const renderMode=()=>{
    ...
    const comparisonMedia=[...host.querySelectorAll(".compare-media")];comparisonMedia.forEach(item=>item.addEventListener(isVideo?"loadedmetadata":"load",()=>requestAnimationFrame(fitComparisonHeight),{once:true}));requestAnimationFrame(fitComparisonHeight);
  ...
  const fitComparisonHeight=()=>{const host=$("#compare-viewport");if(!host)return;const media=[...host.querySelectorAll(".compare-media")],ratios=media.map(item=>{const width=item.naturalWidth||item.videoWidth,height=item.naturalHeight||item.videoHeight;return width>0&&height>0?width/height:0}).filter(Boolean);if(!ratios.length||!host.clientWidth)return;const maxHeight=Math.min(760,Math.max(320,window.innerHeight*.72)),availableWidth=compareState.mode==="side"?Math.max(1,(host.clientWidth-12)/2):host.clientWidth,target=Math.max(320,Math.min(maxHeight,Math.max(...ratios.map(ratio=>availableWidth/ratio))));if(compareState.mode==="side")host.querySelectorAll(".compare-image").forEach(panel=>{panel.style.height=`${target}px`});else{const stage=host.querySelector(".wipe-stage");if(stage)stage.style.height=`${target}px`}};

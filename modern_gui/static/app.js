...
const HELP = {
  ...
  krea2_turbo_lora: "Recommended low-memory alternative to loading a complete Turbo DiT. This frozen RAW-to-Turbo LoRA is enabled only for previews and is never trained or saved into your LoRA. It can be used with block swapping. Leave the full Turbo DiT field empty when using it.",
  krea2_turbo_lora_multiplier: "Strength of the preview-only Turbo delta. Use 1.0 for the published extracted Turbo LoRA unless its author recommends another value.",
  krea2_lora_target_preset: "Controls which Krea2 layers receive adapter weights. All linear layers is the current Musubi/AI Toolkit behavior. Skip text fusion is an experimental community mitigation for unstable text-fusion updates. Attention only also excludes the main MLPs and is the conservative option for long runs or LoRAs that need to stack, but it may learn some concepts more slowly.",
  gradient_checkpointing_cpu_offload: "Moves saved checkpoint activations to system RAM between forward and backward. This can reduce VRAM, especially for Krea 2, but adds CPU transfer overhead and slows training. It only has an effect when Gradient Checkpointing is enabled.",
...
function renderGuided() {
  ...
  const modelKeys = mode === "Krea 2"
    ? ["krea2_dit_model","krea2_text_encoder","vae_model","krea2_turbo_dit","krea2_turbo_lora","krea2_projector_diff"]
...
function defaultPromptForMode(){
  const mode=state.settings.training_mode||"Wan 2.2",turbo=mode==="Krea 2"&&(String(state.settings.krea2_turbo_dit||"").trim()||String(state.settings.krea2_turbo_lora||"").trim());

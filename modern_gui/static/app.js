...
const HELP = {
  ...
  minimax_h3_quality_protection_method: "Choose how MiniMax H3 keeps its original generation quality while the new LoRA learns. Dynamic Sigma is the proven default. The Ostris assistant modes are faster alpha experiments.",
  ...
  minimax_h3_training_assistant_enabled: "Keeps Ostris's frozen alpha helper active during training steps. It is disabled for previews and never saved into your LoRA.",
  ...
  minimax_h3_training_assistant: "Frozen alpha helper published by Ostris. The default owner/repository/file value downloads automatically once and is then reused from the Hugging Face cache. You may also paste a local .safetensors path.",
...
const LONG_HELP_COPY = {
  ...
  minimax_h3_training_assistant_enabled: "This attaches Ostris's published alpha helper LoRA to the frozen H3 transformer while your own LoRA trains. The helper stays frozen, is absent from the optimizer, is temporarily disabled for previews, and is never written into your output.\n\nIt can be combined freely with periodic or every-step Dynamic Sigma and with the optional drift check.",
...
function fieldControl(field, {wide = false} = {}) {
  ...
  const commit = () => {
    ...
    if(["minimax_h3_training_assistant_enabled","minimax_h3_dynamic_sigma_enabled","minimax_h3_base_preservation_enabled"].includes(field.key)){
...
function renderComparison(index) {
  ...
  const renderMode=()=>{
    ...
    host.style.setProperty("--compare-zoom",String(compareState.zoom));$("#compare-zoom-label").textContent=`${Math.round(compareState.zoom*100)}%`;
  ...
  const setZoom=value=>{compareState.zoom=Math.max(.5,Math.min(2.5,Number(value)||1));const host=$("#compare-viewport");host?.style.setProperty("--compare-zoom",String(compareState.zoom));$("#compare-zoom-label").textContent=`${Math.round(compareState.zoom*100)}%`};
  ...
  $("#compare-fit").addEventListener("click",()=>setZoom(1));$("#compare-zoom-out").addEventListener("click",()=>setZoom(compareState.zoom-.1));$("#compare-zoom-in").addEventListener("click",()=>setZoom(compareState.zoom+.1));

...
def patch_convrot_int8_modules(
    ...
    *,
    ...
    """Compatibility loader for this fork's direct pruned-ConvRot checkpoint path."""
    _validate_convrot_bwd_mode(bwd_mode)
    expected_paths = set(scale_shapes)
    patched = set()
    for name, module in model.named_modules():
        if not isinstance(module, nn.Linear) or name not in expected_paths:
            continue
        ...
        module.register_buffer(
            "scale_weight", torch.empty(expected_scale_shape, dtype=torch.float32, device=weight.device)
        )
        _patch_convrot_int8_linear(module, groupsize, bwd_mode)
        patched.add(name)
    missing = expected_paths - patched
    if missing:
        raise ValueError(f"ConvRot INT8 checkpoint references unknown Linear modules: {sorted(missing)[:20]}")
    logger.info("Number of direct ConvRot INT8 Linear layers patched: %d", len(patched))
    return model

...
def _load_qwen3_vl_model(
    ...
    *,
    ...
    sd = load_split_weights(model_path, device="cpu", disable_mmap=disable_mmap, dtype=None)
    sd = _convert_comfyui_qwen3vl_state_dict(sd)
    is_prequant_fp8 = any(k.endswith(FP8_SCALE_SUFFIX) for k in sd.keys()) or (
        FP8_WEIGHT_DTYPE is not None and any(v.dtype == FP8_WEIGHT_DTYPE for v in sd.values())
    )

    device = torch.device(device)
    if is_prequant_fp8:
        sd = _prepare_qwen3_vl_fp8_state_dict(sd, device=device, dtype=dtype)
    ...
    model.to(device)
    if dtype is not None and not is_prequant_fp8:
        model.to(dtype)

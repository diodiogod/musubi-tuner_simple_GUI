...
def build_h3_layout(
    *,
    ...
    if task == "fl2va":
        if references:
            raise ValueError("MiniMax-H3 FL2VA layout does not accept reference rows")
        if one_frame:
            ...
            if not 1 <= len(visual_conditions) <= 2:
                raise ValueError("MiniMax-H3 video FL2VA layout requires one or two visual conditions")

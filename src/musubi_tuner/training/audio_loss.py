...
def add_audio_train_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--audio_only",
        action="store_true",
        help="experimental: supervise real target audio only; video remains synchronized model context",
    )
    parser.add_argument(
        "--audio_loss_weight",
        type=float,
...
def effective_audio_loss_weights(audio_present: torch.Tensor, args: argparse.Namespace) -> torch.Tensor:
    ...
    if args.video_only and audio_only:
        raise ValueError("--video_only and --audio_only are mutually exclusive")
    if args.video_only:

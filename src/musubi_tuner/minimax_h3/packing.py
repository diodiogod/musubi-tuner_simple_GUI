...
def build_image_position_grid(
    text_length: int,
    video: H3VideoGeometry,
    *,
    device: torch.device | str | None = None,
) -> torch.Tensor:
    """Position grid for experimental still-image training: ``[text | one video frame]``."""
    if text_length <= 0:
        raise ValueError("MiniMax-H3 image training requires at least one text row")
    if video.frames != 1:
        raise ValueError("MiniMax-H3 image training requires exactly one latent frame")
    frame, _ = _frame_grid(video)
    positions = torch.zeros(1, text_length + frame.shape[0], 3, dtype=torch.float64)
    positions[0, :text_length, 0] = torch.arange(text_length, dtype=torch.float64)
    positions[0, text_length:, 0] = float(text_length)
    positions[0, text_length:, 1:] = frame
    return positions.to(device=device) if device is not None else positions
...
def unpack_video_rows(rows: torch.Tensor, video: H3VideoGeometry) -> torch.Tensor:
    """Public video-only counterpart of :func:`unpack_targets`."""
    return _unpack_video_rows(rows, video)

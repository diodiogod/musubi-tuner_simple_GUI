...
from musubi_tuner.minimax_h3.checkpoint import (
    inspect_convrot_int8_checkpoint,
...
class MiniMaxH3Config:
    ...
    # Comfy's published pruned checkpoints replace the full timestep MLP with a
    # compact sampled curve.  ``None`` is the original BF16 architecture.
...
class AdalnProj(nn.Module):
    ...
    def forward(self, timestep_embeddings: torch.Tensor) -> tuple[torch.Tensor, ...]:
        inputs = F.silu(timestep_embeddings) if self.apply_silu else timestep_embeddings
        # Curve checkpoints store their small AdaLN bases as FP16.  Cast only
        # the tiny timestep matrix for the projection, then return modulation
        # rows in the residual-stream dtype to avoid FP32 promotion.
        projected = self.linear(inputs.to(self.linear.weight.dtype)).to(timestep_embeddings.dtype)
...
def _apply_rope_split_half(hidden_states: torch.Tensor, rotation_table: torch.Tensor) -> torch.Tensor:
    ...
    # Avoid dispatching millions of tiny 2x2 batched GEMMs.  Explicit rotation
    # is equivalent to torch.matmul here, keeps its FP32 accumulation, and is
    # dramatically cheaper for long H3 sequences (upstream issue #1122).
...
class DiTBlock(nn.Module):
    def __init__(self, config: MiniMaxH3Config, *, attn_mode: str, split_attn: bool, dtype, device) -> None:
        ...
        self.adaln_proj = AdalnProj(
            ...
            dtype=torch.float16 if config.adaln_curve_grid is not None else dtype,
            device=device,
        )
...
class FinalLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.adaln_proj = AdalnProj(
            ...
            expand=2,
            modalities=1,
            apply_silu=not use_adaln_curve,
            dtype=torch.float16 if use_adaln_curve else dtype,
            device=device,
        )
    ...
    def forward_video_only(
        ...
        shift, scale = self.adaln_proj(timestep_embeddings)
        video = self.norm(hidden_states)
        video = video * (1.0 + scale[0]) + shift[0]
        return self.video_out(video.to(torch.float32))
...
class MiniMaxH3Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.final_layer = FinalLayer(
            ...
            use_adaln_curve=config.adaln_curve_grid is not None,
            dtype=dtype,
            device=device,
        )
    ...
    def enable_block_swap(self, num_blocks: int, config: BlockSwapConfig) -> None:
        ...
        if self._has_convrot_int8 and not config.h2d_only:
            raise ValueError(
                "Pruned ConvRot INT8 MiniMax-H3 requires H2D-only block swap. Standard block swap exchanges "
                "Linear weights between block objects without their per-channel quantization scales. Enable "
                "--block_swap_h2d_only together with --gradient_checkpointing."
            )
    ...
    def _timestep_embeddings(self, timesteps: torch.Tensor, execution_device: torch.device) -> torch.Tensor:
        timesteps = torch.as_tensor(timesteps, device=execution_device, dtype=torch.float32).reshape(-1)
        if self.config.adaln_curve_grid is None:
            return self.time_embedder(timesteps).to(self.dtype)
    ...
    def forward_image(
        ...
        model_t: float | torch.Tensor,
        text_hidden_states: torch.Tensor,
    ) -> torch.Tensor:
        """Experimental still-image path using only ``[text | target_video]`` rows.

        The audio modules remain loaded for checkpoint compatibility but are never
        executed. LoRAs produced by this path target the shared DiT projections and
        therefore remain applicable to normal H3 video workflows.
        """
        if video_latents.ndim != 5 or video_latents.shape[0] != 1 or video_latents.shape[1] != self.config.in_channels:
            raise ValueError("MiniMax-H3 image latents must be [1,24,1,H,W]")
        if video_latents.shape[2] != 1:
            raise ValueError("MiniMax-H3 image training accepts exactly one latent frame")
        if video_latents.shape[-2] % 2 or video_latents.shape[-1] % 2:
            raise ValueError("MiniMax-H3 image latent height and width must be divisible by 2")
        if text_hidden_states.ndim != 3 or text_hidden_states.shape[0] != 1:
            raise ValueError("MiniMax-H3 image text states must be [1,L,D]")
        if text_hidden_states.shape[-1] not in {self.config.text_dim, self.config.hidden_size}:
            raise ValueError(
                f"MiniMax-H3 image text width must be {self.config.text_dim} or {self.config.hidden_size}"
            )

        execution_device = self.video_patch_proj.weight.device
        output_dtype = video_latents.dtype
        video_latents = video_latents.to(execution_device)
        text_hidden_states = text_hidden_states.to(execution_device)
        text = text_hidden_states.to(self.dtype)
        if text.shape[-1] != self.config.hidden_size:
            text = self.token_refiner(self.condition_proj(text))
        video_rows = pack_video_rows(video_latents)
        video = self.video_patch_proj(video_rows.to(torch.float32)).to(self.dtype)
        hidden_states = torch.cat((text, video), dim=1)

        text_length = text.shape[1]
        geometry = H3VideoGeometry(1, video_latents.shape[-2], video_latents.shape[-1])
        position_ids = build_image_position_grid(text_length, geometry, device=execution_device)
        rotation_table = self._rotation_table(position_ids, hidden_states.dtype)
        timestep_embeddings = self._timestep_embeddings(torch.as_tensor(model_t).reshape(-1)[:1], execution_device)
        modulation_segments = (((0, text_length, 1), (text_length, hidden_states.shape[1], 0)),)
        ...
        for index, block in enumerate(self.blocks):
            ...
            if self.gradient_checkpointing and torch.is_grad_enabled():
                ...
                hidden_states = block(hidden_states, timestep_embeddings, modulation_segments, rotation_table)
            if self.blocks_to_swap:
                self.offloader.submit_move_blocks_forward(self.blocks, index)

        predicted_rows = self.final_layer.forward_video_only(hidden_states[:, text_length:], timestep_embeddings)
        return unpack_video_rows(predicted_rows, geometry).to(output_dtype)
    ...
    def forward(
        ...
        *,
        ...
        unique_timesteps = timestep_rows.unique_timesteps.to(execution_device)
        if self.config.adaln_curve_grid is None:
            timestep_embeddings = self.time_embedder(unique_timesteps).to(self.dtype)
        else:
            table = self.adaln_t_table
            position = unique_timesteps.to(torch.float32).clamp(0.0, 1.0) * (table.shape[0] - 1)
            lower = position.floor().long().clamp(max=table.shape[0] - 2)
            timestep_embeddings = torch.lerp(
                table[lower],
                table[lower + 1],
                (position - lower).unsqueeze(1),
            ).to(self.dtype)
...
def load_h3_transformer(
    ...
    *,
    ...
    if metadata.get("config") is not None:
        config = parse_h3_transformer_config(metadata)
    ...
    # The published Comfy pruned artifact deliberately has no safetensors
    # metadata. Its architecture is self-identifying through the compact AdaLN
    # table and a marker tensor beside every quantized projection.
    scale_shapes, manifest = inspect_convrot_int8_checkpoint(files)
    table_info = manifest.get("adaln_t_table")
    if table_info is None:
        raise ValueError(
            "Metadata-free MiniMax-H3 checkpoints must contain adaln_t_table; "
            "a full BF16 checkpoint needs its published config metadata"
        )
    table_shape, table_dtype = table_info
    if len(table_shape) != 2 or table_shape[0] < 2 or table_shape[1] <= 0 or table_dtype != torch.float32:
        raise ValueError(
            f"MiniMax-H3 pruned adaln_t_table must be rank-2 FP32, got {table_dtype} {tuple(table_shape)}"
        )
    config = MiniMaxH3Config(time_embed_dim=table_shape[1], adaln_curve_grid=table_shape[0])
    expected_quant_modules = {
        f"blocks.{index}.{suffix}"
        for index in range(config.num_layers)
        for suffix in ("attn.qkv_proj", "attn.out_proj", "mlp.fc1", "mlp.fc2")
    }
    actual_quant_modules = set(scale_shapes)
    if actual_quant_modules != expected_quant_modules:
        raise ValueError(
            "MiniMax-H3 pruned checkpoint quantization coverage mismatch: "
            f"missing={sorted(expected_quant_modules - actual_quant_modules)[:20]}, "
            f"unexpected={sorted(actual_quant_modules - expected_quant_modules)[:20]}"
        )

    def factory() -> MiniMaxH3Model:
        model = MiniMaxH3Model(config, attn_mode=attn_mode, split_attn=split_attn, dtype=dtype)
        patch_convrot_int8_modules(model, scale_shapes, bwd_mode=convrot_bwd_mode)
        model._has_convrot_int8 = True
        return model
    ...
    return load_safetensors_module(
        ...
        device=device,
        dtype=None,
        key_transform=lambda key: key.removesuffix(".weight_scale") + ".scale_weight"

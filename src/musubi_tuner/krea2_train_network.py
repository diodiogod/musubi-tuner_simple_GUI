...
class Krea2NetworkTrainer(NetworkTrainer):
    ...
    def _apply_sample_projector_diff(self, model, args):
        if not args.projector_diff:
            return
        target = self._named_live_tensors(model)[krea2_utils.KREA2_PROJECTOR_WEIGHT_KEY]
        if self._sample_projector_diff is None:
            self._sample_projector_diff = krea2_utils.load_krea2_projector_diff(
                args.projector_diff, args.projector_diff_strength, device="cpu", dtype=torch.float32
            )
        if self._sample_projector_backup is None:
            self._sample_projector_backup = target.detach().to("cpu", dtype=target.dtype).clone()
        merged = (target.data.to(torch.float32) + self._sample_projector_diff.to(target.device)).to(target.dtype)
        target.data.copy_(merged)

    def _restore_sample_projector_diff(self, model):
        if self._sample_projector_backup is None:
            return
        target = self._named_live_tensors(model)[krea2_utils.KREA2_PROJECTOR_WEIGHT_KEY]
        target.data.copy_(self._sample_projector_backup.to(target.device))
        self._sample_projector_backup = None
    ...
    def process_batch(
        ...
        if args.depth_anchor_weight > 0:
            from musubi_tuner.perceptual.depth_anchor import reconstruct_clean_latents, resize_latents_for_depth_decode

            predicted_clean = reconstruct_clean_latents(noisy_input, output.pred, timesteps)
            vae_ratio = int(getattr(vae, "spatial_compression_ratio", 8))
            ...
            total = total + args.depth_anchor_weight * depth_loss
    ...
    def compute_auxiliary_loss(self, args, accelerator, transformer, network):
        ...
        return compute_dop_loss(
            ...
            embedding_key="krea2_vl_embed",
            dop_embedding_key="dop_krea2_vl_embed",
        )
...
def krea2_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional tiny Krea 2 projector diff patch (.safetensors). Applied to the RAW training base model "
        "and also to optional Turbo sample generation so previews stay consistent.",
    )
    parser.add_argument(
        "--projector_diff_strength",
        type=float,
        default=1.0,
        help="Multiplier for --projector_diff (e.g. 2.5).",
    )
    return parser
